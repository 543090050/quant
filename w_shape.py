...
for data_range in range(15, len(history_data) + 1):  # data_range 确定游标范围长度  默认从15开始
    logging.info(
        '游标长度=====================================================================================:' + str(data_range))
    for data_range_index in range(1, len(history_data) - data_range + 1):  # 用 游标范围 遍历 总数据
        # 得到游标范围内的df
        ...
        for split in range(2, data_range - 1):
            ...
            # min3_index = region3['low'].idxmin()
            # min3_data = region3.loc[min3_index]
            # min3 = min3_data['low']
            ...
            if high1_index < min1_index < high2_index < min2_index:  # 时间顺序
                if min1 <= min2 and high1 > high2:  # w 形态的四个主要点，高一低一；高二低二
                    if high2 > min2_data['high'] and buy_day['low'] > min2:
            ...
            if top1_flag and bottom1_flag:
                if history_data.index.get_loc(high1_index) - history_data.index.get_loc(
                        min1_index) > 3:  # 高一和低一之间至少有 3 条k线。如果包括两个极点，就是5条k线
